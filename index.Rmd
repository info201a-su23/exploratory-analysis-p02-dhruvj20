...
- Hao Zhu (zh1117@uw.edu)Names of your team members
=======
Dhruv Jagannath (dhruvj20@uw.edu)
Vy Nguyen (nvy24@uw.edu)
Pagna Keo (pagnakeo@uw.edu)
Hao Zhu (zh1117@uw.edu)
>>>>>>> 747a905117472554d7b5fe15d786bf7aee8f67dd
...
Source: https://uw.hosted.panopto.com/Panopto/Pages/Viewer.aspx?id=c470c37e-a4fd-40d7-8c78-b0530123d189

I chose a bar graph to represent the relationship between movie license/rating (a categorical variable) and the average worldwide sales in dollars (a continuous variable). I
...
While all of the movie licenses make the same income on the scale of hundreds of millions (according to this dataset), rated R movies from this dataset perform the lowest in terms of mean worldwide sales with a value of ```mean_money_film %>% filter(License == "R") %>% select(avg_revenue) %>% pull()``` dollars. 

G-rated and PG-13-rated films from this dataset averaged almost identically at ```mean_money_film %>% filter(License == "G") %>% select(avg_revenue) %>% pull()``` dollars and ```mean_money_film %>% filter(License == "PG-13") %>% select(avg_revenue) %>% pull()``` dollars respectively, with PG-rated films from this dataset averaging slightly less at ```mean_money_film %>% filter(License == "PG") %>% select(avg_revenue) %>% pull()``` dollars. 
